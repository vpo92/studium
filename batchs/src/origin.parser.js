...
class OriginParser {
  ...
  static buildOrigin(json) {
    if (json) {
      ...
      let birthPlace = UtilParser.findProperty(
        json,
        'prosop.person.geo-origin.birthplace.data'
      );
      if (birthPlace) {
        origin.birthPlace = OriginParser.parsePlace(birthPlace);
      }

      let diocese = UtilParser.findProperty(
        json,
        'prosop.person.geo-origin.diocese.data'
      );
      if (diocese) {
